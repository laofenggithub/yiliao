...
public class SearchActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		lvPerson.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				SimpleAdapter kindAdapter = new SimpleAdapter(
						SearchActivity.this, ltKind, R.layout.kindlist,
						new String[] { "kind", "img", "status" }, new int[] {
								R.id.kind, R.id.img, R.id.status });
		...
		lvKind.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (kind.get("status").toString().trim().equals(getString(R.string.status))) {
					...
					intent.putExtra("date", mapClick.get("date").toString()
							.trim());
					intent.putExtra("kananame", mapClick.get("kananame")
							.toString().trim());
					intent.putExtra("kajiname", mapClick.get("kajiname")
							.toString().trim());
					intent.putExtra("age", mapClick.get("age").toString()
							.trim());
					intent.putExtra("sex", mapClick.get("sex").toString()
							.trim());
					intent.putExtra("birth", mapClick.get("birth").toString()
							.trim());
					intent.putExtra("pslid", mapClick.get("pslid").toString()
							.trim());
					intent.putExtra("mano", mapClick.get("mano").toString()
							.trim());
					intent.putExtra("mccrse", mapClick.get("mccrse").toString()
							.trim());
					intent.putExtra("mccrsename", mapClick.get("mccrsename")
							.toString().trim());
					intent.putExtra("filename", kind.get("filename").toString()
							.trim());
					intent.putExtra("kindname", kind.get("kind").toString()
							.trim());
		...
		// 受診者検索 click
		btiSearch.setOnClickListener(new Button.OnClickListener() {
			@Override
			public void onClick(View v) {
				PersonAdapter tempAdapter = null;
				if (!edText.getText().toString().trim().equals("")) {
					// search user return list
					ltTemp = searchData(ltPerson, edText.getText().toString()
							.trim());
					if (ltTemp != null) {
				...
				List<Map<String, Object>> nodatalist = new ArrayList<Map<String, Object>>();
				SimpleAdapter kindAdapter = new SimpleAdapter(
						SearchActivity.this, nodatalist, R.layout.kindlist,
						new String[] { "kind", "img", "status" }, new int[] {
								R.id.kind, R.id.img, R.id.status });
				lvKind.setAdapter(kindAdapter);
			}
		...
		btiExit.setOnClickListener(new Button.OnClickListener() {
			@Override
			public void onClick(View v) {
				// 終了
	...
	private List<Map<String, Object>> getPersonData() {
		...
		String info = MyToolKit.readFileSdcard(this
				.getString(R.string.path_person));
	...
	private List<Map<String, Object>> getKindFileNameList() {
		...
		// 暂时改造去掉读文件功能
		// String info =
		// readFileSdcard(this.getString(R.string.path_questions));
		// String[] infoArray=info.split(this.getString(R.string.str_split));
	...
	private List<Map<String, Object>> getKindData(Map<String, Object> person,
			List<Map<String, Object>> kindname) {
	...
	public class PersonAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (holder == null) {
				...
				holder.emptyColumn = (TextView) view
						.findViewById(R.id.emptyColumn);
				holder.personLayout = (LinearLayout) view
						.findViewById(R.id.PersonListLayout);
				view.setTag(holder);
			}
			...
			if (position == selectItem) {
				...
				holder.personLayout
						.setBackgroundColor(Color.rgb(202, 225, 255));
			} else {
